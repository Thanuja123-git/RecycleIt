...
public class MyAdapter extends RecyclerView.Adapter<MyAdapter.HoldView> {
    ...
    @Override
    public void onBindViewHolder(@NonNull  MyAdapter.HoldView holder, @SuppressLint("RecyclerView") int position) {
        Glide.with(ct).load(list.get(position).getImg())
                .placeholder(R.drawable.ic_launcher_background).into(holder.iv);
        holder.name.setText(list.get(position).getName());
        holder.plastic.setText(list.get(position).getPlastic());
    ...
    public class HoldView extends RecyclerView.ViewHolder {
        ImageButton del;
        TextView name,plastic,number;
        ImageView iv;
        public HoldView(@NonNull View itemView) {
            super(itemView);
            del = itemView.findViewById(R.id.del);
            name = itemView.findViewById(R.id.name);
            plastic = itemView.findViewById(R.id.plastic);
